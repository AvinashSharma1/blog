...
class QueryFilter
{
    ...
    private function __construct($query, $params)
    {
        $this->query    = $query;
        $this->params   = $params;
    }
     ...
     *
     * @param array $query A query object to be used as basis for filters
     ...
     *
     * @return boolean Indicates whether the filter has been applied or not
     */ 
    ...
    public function useDateFilters($filters)
    {
        ...
        foreach($filters AS $key => $val)
        {
            ...
            {
                case self::FROM_DATE:
                    $date = DbHelper::validateDate($this->params[$key]) ?
                    $this->params[$key] :
                    DbHelper::getFromDate($this->params[$key]);
                    break;
                case self::TO_DATE:
                    $date = DbHelper::validateDate($this->params[$key]) ?
                    $this->params[$key] :
                    DbHelper::getToDate($this->params[$key]);
                    break;
                default:
            }

            $this->query->where($field, $operator, $date);
        }
     ...
     * @return \Closure
     */
    public function createCallableFilter($where, $orWhere, $operator = '=')
    {
        list($fromField, $fromValue) = each($where);
        list($orFromField, $orFromValue) = each($orWhere);

        return function($query) use ($fromField, $fromValue, $orFromField, $orFromValue, $operator) {
            $query->where(DB::raw('LOWER('.$fromField.')'), $operator, strtolower($fromValue))
                ->orWhere(DB::raw('LOWER('.$orFromField.')'), $operator, strtolower($orFromValue));
            };
    }
    
    /**
     * This function is created for Update the Parameters
     * 
     * @param Array $updatedValues
     */
    public function updateParameters($updatedValues = array())
    {
        foreach ($updatedValues as $key => $value)
        {
            if(isset($this->params[$key]) && ($this->params[$key] === 0 || !empty($this->params[$key])))
            {
